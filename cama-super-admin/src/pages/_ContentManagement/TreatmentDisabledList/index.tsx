...
const routeTag: RouteTag = 'CONTENT_TREATMENT_DISABLED_LIST';

function ContentTreatmentDisabledListPage() {
  ...
  const fetchData = (page: number = 1, searchType: SearchType = defaultSearchValue, searchedValue: string ='') => {
    doctorContentsApi.
    fetchDoctorContentsDisabledList(page, searchType, searchedValue)
      .then(res => {
              ...
              {list.map((d, idx) => (
                ...
                >
                  <TableCell align="left" style={{ display: 'flex', cursor: 'pointer' }}>
                    {d.image === null && <EmptyThumbImg />}
                    {d.image !== null && <ThumbImg src={d.image} alt="thumb"/>}
                    <ContentView>
...
export default ContentTreatmentDisabledListPage;
