...
# Legacy wrapper: dataset moved to D:\food_mvp (SSD). Prefer _resume_on_d.ps1.
Write-Host ("==== resume batch=48 workers=4 on D: (via legacy _resume_on_f) " + (Get-Date -Format o) + " ====")
...
# 7B: raise VRAM use (~11-12GB) on student n; teacher s uses half (capped at 32)
