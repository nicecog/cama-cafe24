...
export default function MyPage() {
  ...
  const cancerInfoGuideCloseGuardUntil = useAtomValue(
    cancerInfoGuideCloseGuardUntilAtom,
  );
  ...
  const onClickHandler = (index: number) => {
    ...
    if (index === 4) {
      setConsultationInquiryOpen(true);
    }
    if (index === 5) {
  ...
  const setMainOpen = (nextOpen: boolean) => {
    if (
      !nextOpen &&
      (hasChildPopupOpen ||
        mainNestedDialogOpenRef.current ||
        Date.now() < cancerInfoGuideCloseGuardUntil ||
        Date.now() < ignoreMainCloseUntilRef.current)
    ) {
      return;
    }
    setOpen(nextOpen);
  ...
  const setPolicyOpen = (nextOpen: boolean) => {
    if (!nextOpen) guardMainClose();
    setPolicyView((prev) => ({ ...prev, open: nextOpen }));
  };

  const setQrOpen = (nextOpen: boolean) => {
    if (!nextOpen) guardMainClose();
    setQrPopupOpen(nextOpen);
  };

  const setConsultationOpen = (nextOpen: boolean) => {
    if (!nextOpen && consultationNestedDialogOpenRef.current) {
      guardMainClose();
      return;
    }
    if (!nextOpen) guardMainClose();
    setConsultationInquiryOpen(nextOpen);
  };

  const setDoctorTransferDrawerOpen = (nextOpen: boolean) => {
    if (!nextOpen) guardMainClose();
  ...
  const onLogout = () => {
    mainNestedDialogOpenRef.current = true;
    void confirm(
      {
        title: "로그아웃",
        body: "로그아웃 하시겠습니까?",
      ...
      guardMainClose();
      window.setTimeout(() => {
        mainNestedDialogOpenRef.current = false;
      }, CHILD_CLOSE_GUARD_MS);
    });
  };
  ...
  return (
    <>
      ...
      {/* 나의 QR 코드 팝업 */}
      <MyQrcode open={qrPopupOpen} setOpen={setQrOpen} />
      <Popup
        open={consultationInquiryOpen}
        setOpen={setConsultationOpen}
        title="진찰시 문의사항"
      >
        <ConsultationInquiryPage
          onNestedDialogOpenChange={(dialogOpen) => {
            consultationNestedDialogOpenRef.current = dialogOpen;
          }}
        />
      </Popup>
      <Popup
        open={doctorTransferOpen}
        setOpen={setDoctorTransferDrawerOpen}
        title="의사앱 자료전송"
      >
        <DoctorTransferPage
      ...
      <Popup open={open} setOpen={setMainOpen} direction="right" title="내정보">
        ...
        <div className="flex flex-col h-full overflow-y-auto bg-gradient-to-b from-gray-50 to-white hide-scrollbar">
          ...
          <div className="px-4 pb-6 flex flex-col gap-5">
            <button
              ...
              className="group mt-4 flex w-full items-center justify-between rounded-xl border-2 border-gray-100 bg-white p-4 text-left shadow-sm transition hover:border-red-200 hover:bg-red-50/70 disabled:opacity-50"
            >
