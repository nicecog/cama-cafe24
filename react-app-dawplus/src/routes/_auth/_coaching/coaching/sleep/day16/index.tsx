...
import { type Day16Step1Data, getDay16Averages } from "./utils";

export const Route = createFileRoute("/_auth/_coaching/coaching/sleep/day16/")({
...
function RouteComponent() {
  ...
  const [step1, setStep1] = useState<Day16Step1Data>({
    time: "0",
    rating: "0",
    answer: "",
  });

  const averages = getDay16Averages(answerList);
  ...
  const onBeforeNext = (currentStep: number) => {
    if (currentStep === 1 && !step1.answer) {
      toast({
        description: pt("step1.msg_010"),
      });
      return false;
    }
  ...
  return (
    <DayStepFlow
      ...
      showNextButton={(currentStep) =>
        currentStep !== 1 || Boolean(step1.answer)
      }
    >
      <Day16Step1 step1={step1} averages={averages} onChange={setStep1} />
      <Day16Step2 accountName={accountName} />
      <Day16Step3 />
    </DayStepFlow>
  );
}
