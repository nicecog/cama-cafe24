...
import day11Pic from "@/assets/images/coaching/meal/day11/day11.png";
...
interface Day11Step2Props {
  step1: string;
}

export function Day11Step2({ step1 }: Day11Step2Props) {
  const accountName = useAccountName();
  const { pt } = usePageTranslation("coaching/meal/day11");
  const isYes = step1.trim() === pt("MSG_007");

  const items = [
    { title: pt("MSG_011"), description: pt("MSG_024") },
    { title: pt("MSG_012"), description: pt("MSG_025") },
    { title: pt("MSG_013"), description: pt("MSG_026") },
  ...
  return (
    <CoachingInfoStep title={pt("MSG_010")} image={day11Pic}>
      <div className="flex flex-col gap-10 pt-4 pb-12">
        ...
        <div className="space-y-3 px-1 text-center">
          ...
          <p className="text-base font-bold text-slate-500 break-keep">
            {isYes ? pt("MSG_032") : pt("MSG_033")}
          </p>
        </div>

        {/* 외식 전략 리스트 섹션: 카드 레이아웃 적용 */}
