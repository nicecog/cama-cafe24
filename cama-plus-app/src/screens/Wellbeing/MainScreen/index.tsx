...
const WellbeingMainScreen: React.FC<
  MainBottomTabNavigationProps<'WellbeingMainScreen'>
> = ({ navigation, route }) => {
  ...
  const wellbeingUri = useMemo(() => {
    if (paramsCategoryCd === 'M' && account?.loginId) {
      return patientWebviewUrls.wellbeing(account.loginId);
    }
    return patientWebviewUrls.help();
  }, [paramsCategoryCd, account?.loginId]);

  const webViewSource = usePatientWebViewSource(wellbeingUri);
  const routesParams = useRoute();

  const onPressHardwareBackButton = () => {
    //console.log(' routesParams.name =>' + routesParams.name);
    if (routesParams.name === 'WellbeingMainScreen') {
  ...
  //const uri =
  //  'https://www.figma.com/proto/CvHPG4RwkmE9mZ3LjJACc1/20230908_CAMA-Plus-%EB%B3%B5%EC%82%AC%EB%B3%B8?page-id=333%3A3864&type=design&node-id=504-5536&viewport=1374%2C-922%2C0.14&t=uwrqbJRFYggHVdhJ-1&scaling=scale-down&starting-point-node-id=740%3A4317&show-proto-sidebar=1';
...
export default WellbeingMainScreen;
