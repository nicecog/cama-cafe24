#!/usr/bin/env python3
"""Apply account_consultation_inquiry DDL on Cafe24 VPS."""
...
SQL_FILE = DEPLOY_ROOT / "sql" / "cafe24-consultation-inquiry.sql"
...
def main() -> None:
    if not SQL_FILE.is_file():
        raise SystemExit(f"Missing SQL: {SQL_FILE}")
    ...
    result = psql(client, SQL_FILE.read_text(encoding="utf-8"))
    if result.strip():
        print(result)

    verify = """
SELECT 'account_consultation_inquiry' AS tbl, count(*)::text AS cnt
  FROM information_schema.tables
 WHERE table_schema='public' AND table_name='account_consultation_inquiry'
UNION ALL
SELECT 'account_consultation_inquiry_history', count(*)::text
  FROM information_schema.tables
 WHERE table_schema='public' AND table_name='account_consultation_inquiry_history';
"""
    print(psql(client, verify))
    client.close()
    print("Consultation inquiry schema applied.")
