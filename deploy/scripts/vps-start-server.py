...
cmds = [
    "docker ps -a --format '{{.Names}} {{.Status}}' | grep cama",
    "cd /opt/cama/deploy && docker-compose -f docker-compose.cafe24.yml --env-file .env.cafe24 up -d cama-plus-server",
    "sleep 20",
    "docker ps --format '{{.Names}} {{.Status}}' | grep cama-plus-server",
